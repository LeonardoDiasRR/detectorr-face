...
class TimestampVO:
    ...
    def iso_format(self) -> str:
        """
        Retorna o timestamp no formato ISO 8601 com informações de timezone.
        
        Alias para iso_format_with_tz().

        :return: String no formato ISO 8601 com timezone (ex: 2025-12-21T11:30:00+00:00).
        """
        return self.iso_format_with_tz()

    def iso_format_with_tz(self) -> str:
        """
        Retorna o timestamp no formato ISO 8601 com informações de timezone.
        
        Se o datetime for ingênuo (sem timezone), assume UTC.
        Se for timezone-aware, retorna com o timezone original.

        :return: String no formato ISO 8601 com timezone (ex: 2025-12-21T11:30:00+00:00).
        """
        if self._value.tzinfo is None:
            # Se é ingênuo, assume UTC
            dt_with_tz = self._value.replace(tzinfo=timezone.utc)
        else:
            # Se já tem timezone, mantém o original
            dt_with_tz = self._value
